...
def within_aria_label(aria_label, &block)
  within("[aria-label='#{aria_label}']", &block)
end
...
When('the user clicks on {string} within {string}') do |string, aria_label|
...
Given('the user is on the page for the {string}') do |class_name|
  visit(polymorphic_path(@data[class_name]))
end
